...
{    
    ...
    if ($self->cove_mode())
    {
        my $ns_cm = $self->{mainNS_cm_file_path}->{Domain};
        if ($opts->general_mode() or $opts->organelle_mode())
        {
    ...
    elsif ($self->infernal_mode()) 
    {
       ($hmm_score, $hit_start, $hit_end, $hit_ct) = 
            $self->cmsearch_hmm_score($global_constants, $opts, $log, $tmp_trnaseq_file, $prescan_tRNA_id, $trna->model());
...
sub decode_nci_tRNA_properties
{
    ...
    if ($self->cove_mode() || $self->infernal_mode())
    {
        ...
        for (my $i = 0; $i < scalar(@introns); $i++)
        {
            if ($acodon_index > $introns[$i]->{rel_end})
            {
...
sub fix_fMet
{
    ...
    if ($inf->{score} > 50 and $trna->isotype() eq "Met" and $opts->bact_mode())
    {
        ...
        while ($cm_tRNA->seqname() ne "")
        {
            ...
            else
            {
                ...
                for (my $i = 0; $i < scalar(@ar_introns); $i++)
                {
                    ...
                    elsif ($ar_introns[$i]->{type} eq "CI" and length($ar_introns[$i]->{seq}) > 40 and
...
# Runs Infernal cmsearch, and returns the hmm score
sub cmsearch_hmm_score
{
    my $self = shift;
    my ($global_constants, $opts, $log, $tmp_trnaseq_file, $seq_name, $type) = @_;

    my ($cms_output_file, $cur_cm_file, @cms_lines,
        $subseq_start, $subseq_end, $evalue, $score, $besthit_score, $line,
        $besthit_start, $besthit_end, $hit_ct);

    $cur_cm_file = $self->{mainNS_cm_file_path}->{$type};
    $cms_output_file = $global_constants->get("temp_dir")."/tscan$$"."_cm_hmm.out";
    my $scan_flag = 0;
        ...
        while (($cm_tRNA->seqname() ne "") and ($cm_tRNA->seqname() eq $prescan_tRNA->seqname().".t".&pad_num($prescan_tRNA->id(), 6)))
        {
            ...
            else
            {
                    ...
                    ($self->{main_cm_file_path}->{Domain} ne $global_constants->get_subvalue("cm", "general")))
                {
                    ...
                    (substr($cm_tRNA->seq(), length($cm_tRNA->seq()) - 5, 2) =~ /[acgt][ACGT]/)) ||
                    ((substr($cm_tRNA->ss(), length($cm_tRNA->ss()) - 7) eq "<......") &&
                    (substr($cm_tRNA->seq(), length($cm_tRNA->seq()) - 6, 3) =~ /[acgt]{2}[ACGT]/))) &&
                    ($self->{main_cm_file_path}->{Domain} ne $global_constants->get_subvalue("cm", "eukaryota")) &&
                    ($self->{main_cm_file_path}->{Domain} ne $global_constants->get_subvalue("cm", "general")))
                {
                    my $trim_len = 4;
                    if (substr($cm_tRNA->ss(), length($cm_tRNA->ss()) - 7) eq "<......")
                    {
						$trim_len = 5;
					}
